...
public interface ShadowSockMapper {
    ...
    @Update("update shadow set status=#{status}, check_in_time=#{checkInTime} where url=#{url}")
    ...
    @Insert("insert into shadow (url, login_uri, check_in_uri, status) values (#{url}, #{loginUri}, #{checkInUri}, 1)")
